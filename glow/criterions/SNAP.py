...
from utils.data_utils import lookahead_type, lookahead_finished, lookahead_type_name
from utils.snip_utils import group_snip_forward_linear, group_snip_conv2d_forward, group_snip_invconv2d_forward
...
class SNAP(General):
    ...
    def prune(self, percentage, train_loader=None, local=False, manager=None, **kwargs):

        all_scores, grads_abs, log10, norm_factor, vec_shapes = self.get_weight_saliencies(train_loader)

        self.handle_pruning(all_scores, grads_abs, norm_factor, percentage, local)

    def handle_pruning(self, all_scores, grads_abs, norm_factor, percentage, local):
    ...
    def insert_governing_variables(self, net):
        ...
        gov_out = None
        do_avg_pool = 0
        for name, layer in lookahead_type_name(net.named_modules()):
            is_conv = isinstance(layer, nn.Conv2d)
            is_invconv = 'invconv' in name
            is_fc = isinstance(layer, nn.Linear)
            is_avgpool = isinstance(layer, nn.AdaptiveAvgPool2d)
            is_actnorm = 'actnorm' in name

            if is_avgpool:
                do_avg_pool = int(np.prod(layer.output_size))

            elif is_conv or is_fc or is_invconv:

                if is_invconv:
                    out_dim, in_dim = layer.calc_weight().shape[:2]
                else:
            ...
            if is_invconv:
                layer.forward = types.MethodType(group_snip_invconv2d_forward, layer)
