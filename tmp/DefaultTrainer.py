    ...
    def _epoch_iteration(self):
        ...
        if True:
            ...
            if len(self.gradient_adtest) != 0:
                ...
                self._metrics.add(stats.kstest(self.gradient_adtest, gradient_adtest, mode='exact')[1].astype(np.float32), key="criterion/pvalue")
                self._metrics.add(cucconi_test(np.asarray(self.gradient_adtest), np.asarray(gradient_adtest))[1], key="criterion/cucconipvalue")
                self._metrics.add(stats.kstest(self.gradient_adtest, gradient_adtest, mode='exact')[0], key="criterion/kstest")
