...
class NEW(General):
    ...
    def handle_pruning(self, all_scores, grads_abs, log10, manager, norm_factor, percentage):
        ...
        for name, grad in grads_abs.items():
            scores = torch.flatten(grad)
            # don't prune more or less than possible
            num_params_to_keep = int(len(scores) * (1 - percentage))
            if num_params_to_keep < 1:
                num_params_to_keep += 1
            elif num_params_to_keep > len(scores):
                num_params_to_keep = len(scores)

            # threshold
            threshold, _ = torch.topk(scores, num_params_to_keep, sorted=True)
            acceptable_score = threshold[-1]
    ...
    def get_weight_saliencies(self, train_loader, ood_loader=None):
        ...
        # IF USING THIS SWITCH BACK TO SAVED BATCHES BELOW !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        #
        #     if i == iterations: break
        #
        #     saved_batches.append((x, y))
        #
        #     adv_results, _ = construct_adversarial_examples(x, y, 'FGSM', net,
        #                                                     self.model.device,
        #                                                     12, False, False)
        #     _, advs, _ = adv_results
        #
        #     inputs = advs.to(self.model.device)
        ...
        #         grads_adv[name + ".weight"] = layer.weight.grad.clone()
        #
        # net.zero_grad()
        for i, (x, y) in enumerate(train_loader):
        ...
        for name, layer in net.named_modules():
            if "Norm" in str(layer): continue
            if name + ".weight" in self.model.mask:
                grads[name + ".weight"] = layer.weight.grad.clone()
        ...
        for name, layer in net.named_modules():
            if "Norm" in str(layer): continue
            if name + ".weight" in self.model.mask:
                grads_ood[name + ".weight"] = layer.weight.grad.clone()

        grads_dist = {}
        for name, layer in net.named_modules():
            if "Norm" in str(layer): continue
            if name + ".weight" in self.model.mask:
                grads_dist[name + ".weight"] = - (grads_ood[name + ".weight"] - grads[name + ".weight"]) ** 2

        all_scores = torch.cat([torch.flatten(x) for _, x in grads_dist.items()])
        norm_factor = 1
        ...
        return all_scores, grads_dist, log10, norm_factor
