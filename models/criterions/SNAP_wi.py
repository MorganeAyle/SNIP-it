...
class SNAP_wi(General):
    ...
    def __init__(self, orig_scores=False, *args, **kwargs):
        super(SNAP_wi, self).__init__(*args, **kwargs)
                ...
                if self.scores is None:
                    ...
                    for name, layer in net.named_modules():
                        ...
                        if hasattr(layer, "gov_in"):
                            for (identification, param) in [(id(param), param) for param in
                                                            [layer.gov_in, layer.gov_out] if
                                                            param.requires_grad]:
                                try:
                                ...
                                if identification not in self.scores:
                                    self.scores[identification] = torch.unsqueeze(grad_ab, 0)
                    ...
                    for name, layer in net.named_modules():
                        ...
                        if hasattr(layer, "gov_in"):
                            for (identification, param) in [(id(param), param) for param in
                                                            ...
                                                            param.requires_grad]:
                                try:
                                    curr_scores = torch.abs(param.grad.data)
                                except:
                                    curr_scores = torch.zeros_like(param.data)

                                if identification not in tmp:
                                    tmp[identification] = 0
                                    self.scores[identification] = torch.cat((self.scores[identification], curr_scores.unsqueeze(0)))

        self.scores_mean = OrderedDict()
        self.scores_std = OrderedDict()

        if self.orig_scores:
            for identification, value in self.scores.items():
                self.scores_mean[identification] = torch.mean(value, dim=0)
                self.scores_std[identification] = torch.std(value, dim=0, unbiased=True)
