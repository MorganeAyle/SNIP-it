...
class Pruneable(GeneralModel):
    ...
    def post_init_implementation(self):
        ...
        with torch.no_grad():
            ...
            if self.is_maskable:
                ...
                for name, module in self.named_modules():
                    if 'conv' in name or 'fc' in name:
                        self.mask[name] = torch.ones_like(module.W_mu.data).to(self.device)
                        # self.mask[name + '.W_rho'] = torch.ones_like(module.W_rho.data).to(self.device)
    ...
    def _get_weight_count(self):
        return sum([tens.flatten().size()[0] for name, tens in self.named_parameters()
                    if (('conv' in name) or ('fc' in name)) and ('W_mu' in name)])
    ...
    def _clone_weights(self, weight_list_reference, bias=False):
        ...
        return {name: tens.data.detach().clone().to(self.device) for name, tens in
                weight_list_reference if ((('conv' in name) or ('fc' in name)) and ('W_mu' in name)) or bias}
        ...
        with torch.no_grad():
            for name, module in self.named_modules():
                if name in self.mask:
                    print(name)
                    module.W_mu.data *= self.mask[name]
                    module.W_rho.data *= self.mask[name]
    ...
    def apply_grad_mask(self):
        ...
        for name, module in self.named_modules():
            if name in self.mask:
                module.W_mu.grad.data *= self.mask[name]
                module.W_rho.grad.data *= self.mask[name]
