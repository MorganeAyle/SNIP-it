...
class SNIP(General):
    ...
    def handle_pruning(self, all_scores, grads_abs, log10, manager, norm_factor, percentage, local):
        ...
        for name, module in self.model.named_modules():
            if name in self.model.mask:
                module.mask1 = self.model.mask[name]
    ...
    def get_weight_saliencies(self, train_loader, ood_loader=None):
        ...
        criterion = metrics.ELBO(len(train_loader)).to(self.device)
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            log_outputs = butils.logmeanexp(torch.unsqueeze(outputs, -1), dim=2)

            loss = criterion(log_outputs, targets, kl, 1.0) / iterations
        ...
        for name, layer in net.named_modules():
            if 'conv' in name or 'fc' in name:
                grads_abs[name] = torch.abs(
                    layer.W_mu.grad * (layer.W_mu.data / (1e-8 + loss_sum.item())))
