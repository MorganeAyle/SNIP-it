...
class BBBLinear(ModuleWrapper):
    ...
    def forward(self, x, sample=True):
        ...
        act_mu = F.linear(x, self.W_mu * self.mask.to(self.W_mu.device), self.bias_mu)
        act_var = 1e-16 + F.linear(x ** 2, (self.W_sigma ** 2) * self.mask.to(self.W_mu.device), bias_var)
