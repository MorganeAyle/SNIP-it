...
class BBBConv2d(ModuleWrapper):
    ...
    def forward(self, x, sample=True):
        ...
        act_mu = F.conv2d(
            x, self.W_mu * self.mask.to(self.W_mu.device), self.bias_mu, self.stride, self.padding, self.dilation, self.groups)
        act_var = 1e-16 + F.conv2d(
            x ** 2, (self.W_sigma ** 2) * self.mask.to(self.W_mu.device), bias_var, self.stride, self.padding, self.dilation, self.groups)
